...
if ($_SESSION['nama_pelanggan'] !== null) {
  ...
  if ($exec) {
    ...
    $datapesanan = mysqli_fetch_all($exec, MYSQLI_ASSOC);
    if ($jumlahdata > 0) {
  ...
  <body>
    <div class="d-flex" id="wrapper">
      ...
      <div id="page-content-wrapper">
        ...
        <!-- Page content-->
        <div class="container">
          <?php if (!isset($_GET['detailpesanan']) && !isset($_GET['form'])) { ?>
                  ...
                  <?php if (isset($_GET['form'])) { ?>
                    <div class="col-12 mt-5">
                      <form action="proses.php" method="post" enctype="multipart/form-data">
                        <input type="hidden" value="<?= $datadetail[0]['kode_transaksi'] ?>" name="kodetransaksi">
                        <div class="mb-3">
                          <label for="formFile" class="form-label">Masukan Bukti Pembayaran</label>
                          <input required class="form-control" type="file" id="formFile" name="foto-pembayaran">
                        </div>
                        <div class="mb-3">
                          <input class="form-control btn-info" type="submit" name="foto" value="Kirim Bukti">
                        </div>
                      </form>
                    </div>
                  <?php } ?>
                  <?php if ($datadetail[0]['foto_pembayaran']) { ?>
                    <div class="col-12 my-4">
                      <a class="form-control btn btn-success text-center text-decoration-none" target="blank" href="https://wa.me/62<?= $nomeradmin ?>">Hubungi Petugas</a>
                    </div>
                  <?php } else { ?>
                    <div class="col-6 my-4">
                      <a class="form-control btn btn-warning text-center text-decoration-none <?= (isset($_GET['form'])) ? 'disabled' : ''; ?>" href="?detailpesanan=<?= $datadetail[0]['kode_transaksi'] ?>&form=<?= $id_pelanggan ?>">Masukan Foto Pembayaran</a>
                    </div>
                    <div class="col-6 my-4">
                      <a class="form-control btn btn-success text-center text-decoration-none" target="blank" href="https://wa.me/62<?= $nomeradmin ?>">Hubungi Petugas</a>
                    </div>
